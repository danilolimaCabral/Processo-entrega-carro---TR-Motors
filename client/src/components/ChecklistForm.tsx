...
type ResponsibleRole = "financeiro" | "administrativo";
...
function ChecklistSection({ title, role, saleRecordId, items, onChanged }: ChecklistSectionProps) {
    ...
    <Card>
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        ...
        <Dialog open={isOpen} onOpenChange={setIsOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Novo Item — {title}</DialogTitle>
            </DialogHeader>
      ...
      <CardContent>
        {items.length === 0 ? (
          <div className="text-center py-8 text-slate-500">
            ...
            <p className="text-sm">Adicione itens para que o {title.toLowerCase()} valide</p>
          </div>
        ) : (
...
export function ChecklistForm({ saleRecordId }: ChecklistFormProps) {
  ...
  const administrativoItems = items?.filter((i) => i.responsibleRole === "administrativo") ?? [];
  const financeiroItems = items?.filter((i) => i.responsibleRole === "financeiro") ?? [];
