...
import { CheckCircle2, Loader2, XCircle, FileSearch, User } from "lucide-react";
...
export default function FinanceiroPage() {
  ...
  const { data: records, isLoading } = trpc.sales.listForFinanceiro.useQuery();
  const [selected, setSelected] = useState<SaleRecord | null>(null);
  const [rejectOpen, setRejectOpen] = useState(false);

  const approve = trpc.sales.approveFinanceiro.useMutation({
    onSuccess: () => {
      toast.success("Registro aprovado. Aguardando análise do Administrativo.");
      setSelected(null);
      utils.sales.listForFinanceiro.invalidate();
    },
    onError: (e) => toast.error(e.message),
  });

  const reject = trpc.sales.rejectFinanceiro.useMutation({
    onSuccess: () => {
    ...
    <div className="space-y-8 max-w-5xl">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Analise os documentos e aprove ou reprove os registros pendentes.
        </p>
      </div>
      ...
      <Card className="shadow-sm">
        <CardHeader className="pb-4">
          <div className="flex items-center justify-between">
            <div>
              <CardTitle className="text-base">Aguardando análise financeira</CardTitle>
      ...
      <RejectModal
        ...
        title="Reprovar — Análise Financeira"
      />
    </div>
  );
}
