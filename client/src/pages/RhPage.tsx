            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                {months.map((monthName, index) => {
                  ...
                  return <Card key={month} role={canManageProfile ? "button" : undefined} tabIndex={canManageProfile ? 0 : undefined} onClick={() => canManageProfile && openPjInvoiceForm(month, invoice)} onKeyDown={(event) => { if (canManageProfile && (event.key === "Enter" || event.key === " ")) { event.preventDefault(); openPjInvoiceForm(month, invoice); } }} className={`group relative min-h-[188px] overflow-hidden border shadow-sm transition-all duration-200 ${cardClass} ${canManageProfile ? "cursor-pointer hover:-translate-y-1 hover:shadow-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:ring-offset-2" : ""}`}><div className={`absolute inset-x-0 top-0 h-1 ${accentClass}`} /><CardContent className="flex h-full min-h-[188px] flex-col p-5 pt-6"><div className="flex items-center gap-3"><div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl shadow-sm ${iconClass}`}><CalendarDays className="h-5 w-5" /></div><div className="min-w-0"><p className="break-words text-lg font-extrabold leading-tight text-slate-900">{monthName}</p><p className="mt-1 text-xs font-semibold text-slate-500">Competência · {String(month).padStart(2, "0")}/{invoiceYear}</p></div></div><Badge className={`mt-4 w-fit border px-2.5 py-1 text-[10px] font-bold ${badgeClass}`}>{statusLabel}</Badge>{invoice ? <div className="mt-auto flex items-end justify-between gap-3 border-t border-slate-200/70 pt-4"><div className="min-w-0"><p className="text-lg font-extrabold tracking-tight text-slate-900">R$ {Number(invoice.amount || 0).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}</p><p className="mt-1 flex items-center gap-1.5 text-xs font-medium text-slate-500"><Receipt className="h-3.5 w-3.5 shrink-0" /> NF {invoice.invoiceNumber}</p></div><ChevronRight className="h-5 w-5 shrink-0 text-slate-400 transition-transform group-hover:translate-x-1" /></div> : <div className="mt-auto flex items-center justify-between gap-3 border-t border-slate-200/70 pt-4"><div className="flex min-w-0 items-center gap-2.5"><div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-red-50 text-red-600"><Upload className="h-4 w-4" /></div><div><p className="text-sm font-bold text-slate-700">Enviar nota fiscal</p><p className="text-xs text-slate-500">Clique para preencher</p></div></div><ChevronRight className="h-5 w-5 shrink-0 text-slate-400 transition-transform group-hover:translate-x-1" /></div>}</CardContent></Card>;
                })}
              </div>
