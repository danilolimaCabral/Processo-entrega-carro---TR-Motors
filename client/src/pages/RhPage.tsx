            ...
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                {months.map((monthName, index) => {
                  ...
                  const cardClass = paid ? "border-emerald-200 bg-emerald-50/70" : underReview ? "border-amber-200 bg-amber-50/70" : rejected ? "border-red-200 bg-red-50/70" : "border-slate-200 bg-white";
                  const statusLabel = paid ? "Pago" : underReview ? "Em conferência" : rejected ? "Rejeitada" : "Aguardando NF";
                  const accentClass = paid ? "bg-emerald-500" : underReview ? "bg-amber-500" : rejected ? "bg-red-500" : "bg-slate-300";
                  const iconClass = paid ? "bg-emerald-100 text-emerald-700" : underReview ? "bg-amber-100 text-amber-700" : rejected ? "bg-red-100 text-red-700" : "bg-slate-100 text-slate-500";
                  const badgeClass = paid ? "border-emerald-200 bg-emerald-100 text-emerald-700" : underReview ? "border-amber-200 bg-amber-100 text-amber-700" : rejected ? "border-red-200 bg-red-100 text-red-700" : "border-slate-200 bg-slate-100 text-slate-600";

                  return (
                    <Card
                      key={month}
                      role={canManageProfile ? "button" : undefined}
                      tabIndex={canManageProfile ? 0 : undefined}
                      onClick={() => canManageProfile && openPjInvoiceForm(month, invoice)}
                      onKeyDown={(event) => {
                        if (canManageProfile && (event.key === "Enter" || event.key === " ")) {
                          event.preventDefault();
                          openPjInvoiceForm(month, invoice);
                        }
                      }}
                      className={`group relative min-h-[202px] overflow-hidden border shadow-sm transition-all duration-200 ${cardClass} ${canManageProfile ? "cursor-pointer hover:-translate-y-1 hover:shadow-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-red-500 focus-visible:ring-offset-2" : ""}`}
                    >
                      <div className={`absolute inset-x-0 top-0 h-1 ${accentClass}`} />
                      <CardContent className="flex h-full min-h-[202px] flex-col p-5 pt-6">
                        <div className="flex items-center gap-3">
                          <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl shadow-sm ${iconClass}`}><CalendarDays className="h-5 w-5" /></div>
                          <div className="min-w-0">
                            <p className="break-words text-lg font-extrabold leading-tight text-slate-900">{monthName}</p>
                            <p className="mt-1 text-xs font-semibold text-slate-500">Competência · {String(month).padStart(2, "0")}/{invoiceYear}</p>
                          </div>
                        </div>
                        <Badge className={`mt-4 w-fit border px-2.5 py-1 text-[10px] font-bold ${badgeClass}`}>{statusLabel}</Badge>
                        {invoice ? (
                          <div className="mt-auto flex items-end justify-between gap-3 border-t border-slate-200/70 pt-4">
                            <div className="min-w-0">
                              <p className="text-lg font-extrabold tracking-tight text-slate-900">R$ {Number(invoice.amount || 0).toLocaleString("pt-BR", { minimumFractionDigits: 2 })}</p>
                              <p className="mt-1 flex items-center gap-1.5 text-xs font-medium text-slate-500"><Receipt className="h-3.5 w-3.5 shrink-0" /> NF {invoice.invoiceNumber}</p>
                            </div>
                            <Button type="button" variant="ghost" size="icon" aria-label={`Abrir NF de ${monthName}`} onClick={(event) => { event.stopPropagation(); openPjInvoiceForm(month, invoice); }}><ChevronRight className="h-5 w-5 text-slate-400" /></Button>
                          </div>
                        ) : (
                          <div className="mt-auto flex items-center justify-between gap-3 border-t border-slate-200/70 pt-4">
                            <div className="flex min-w-0 items-center gap-2.5">
                              <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-red-50 text-red-600"><Upload className="h-4 w-4" /></div>
                              <div><p className="text-sm font-bold text-slate-700">Enviar nota fiscal</p><p className="text-xs text-slate-500">Competência do mês</p></div>
                            </div>
                            {canManageProfile && <Button type="button" size="sm" className="shrink-0 bg-red-600 hover:bg-red-700" onClick={(event) => { event.stopPropagation(); openPjInvoiceForm(month); }}>Preencher</Button>}
                          </div>
                        )}
                      </CardContent>
                    </Card>
              ...
              <Dialog open={showPjInvoiceForm} onOpenChange={setShowPjInvoiceForm}>
                <DialogContent className="max-h-[calc(100vh-2rem)] max-w-lg overflow-y-auto">
                  <DialogHeader>
                    <DialogTitle>NF PJ de {months[pjInvoiceForm.competenceMonth - 1]} · {invoiceYear}</DialogTitle>
                  </DialogHeader>
                  <p className="text-sm text-slate-500">Informe os dados da nota e envie o arquivo para a fila do Financeiro.</p>
                  <div className="grid gap-3 sm:grid-cols-2">
                    <div className="space-y-1.5"><Label>Número da NF *</Label><Input value={pjInvoiceForm.invoiceNumber} onChange={(event) => setPjInvoiceForm({ ...pjInvoiceForm, invoiceNumber: event.target.value })} placeholder="Ex.: 000123" /></div>
                    <div className="space-y-1.5"><Label>Valor total *</Label><Input type="number" min="0.01" step="0.01" value={pjInvoiceForm.amount} onChange={(event) => setPjInvoiceForm({ ...pjInvoiceForm, amount: event.target.value })} placeholder="0,00" /></div>
                    <div className="space-y-1.5"><Label>Data de emissão</Label><Input type="date" value={pjInvoiceForm.issueDate} onChange={(event) => setPjInvoiceForm({ ...pjInvoiceForm, issueDate: event.target.value })} /></div>
                    <div className="space-y-1.5"><Label>Arquivo da NF *</Label><Input type="file" accept="application/pdf,image/jpeg,image/png,image/webp" onChange={(event) => setPjInvoiceForm({ ...pjInvoiceForm, file: event.target.files?.[0] || null })} /></div>
                  </div>
                  <div className="space-y-1.5"><Label>Observações para o Financeiro</Label><Textarea value={pjInvoiceForm.notes} onChange={(event) => setPjInvoiceForm({ ...pjInvoiceForm, notes: event.target.value })} placeholder="Informações adicionais para conferência" /></div>
                  <div className="flex flex-col-reverse gap-2 sm:flex-row sm:justify-end">
                    <Button type="button" variant="outline" onClick={() => setShowPjInvoiceForm(false)}>Cancelar</Button>
                    <Button disabled={submitPjInvoice.isPending} onClick={submitMonthlyPjInvoice} className="bg-red-600 hover:bg-red-700">{submitPjInvoice.isPending ? "Enviando..." : "Enviar para conferência financeira"}</Button>
                  </div>
                </DialogContent>
              </Dialog>
            </div>
